...
void* check_broadcast(Thread* send_t, Thread*targ_t,int cnt){/*check recover time*/
    ...
    while(1){
        for(int i = 0; i<cnt; i++){
            ...
            arp((void*)&send_t[i],(void*)&targ_t[i],(void*)&send_t[i]);
            arp((void*)&send_t[i],(void*)&targ_t[i],(void*)&targ_t[i]);
            ...
            if(recv_packet->eth_.type()!=0x806){//not the arp - relay
                for(Thread t :tt){
                    ...
                    flag3 = ((EthIpPacket *)packet)->s_ip == (Ip)htonl(targ_t[i].sender->ip_) ? 1:0;

                    if(flag1&&flag2&&flag3){
                        u_char * relay_packet = (u_char*)malloc(header->caplen);
                        memcpy(relay_packet,recv_packet,header->caplen);

                        ((EthArpPacket*)relay_packet)->arp_.smac() = t.me->mac_;
                        ((EthArpPacket*)relay_packet)->arp_.tmac() = t.target->mac_;
                        ((EthArpPacket*)relay_packet)->eth_.smac() = t.me->mac_;
            ...
            for(Thread t :tt){
                ...
                if(flag1||flag2)
                    arp((void*)&send_t[i],(void*)&targ_t[i],(void*)&t);
            }
        }
    }
}
